...
import { Animated, AppState, Image, Platform, StyleSheet, Text, TouchableOpacity, View, } from 'react-native';
import { Camera, useCameraDevice, useCameraPermission } from 'react-native-vision-camera';
import { useAutoCapture } from '../hooks/useAutoCapture';
import { getDeviceTierConfig } from '../utils/deviceTier';
...
export function DocumentScanner({ onCapture, onCancel, title, frameRatio = 1.41, forceHorizontal = true }) {
    const { hasPermission, requestPermission } = useCameraPermission();
    const device = useCameraDevice('back');
    const cameraRef = useRef(null);
    const [scanState, setScanState] = useState('searching');
    const [tierConfig, setTierConfig] = useState({ intervalMs: 800, stabilityDelayMs: 700 });
    ...
    const captureQuality = useCallback(async () => {
        ...
        try {
            ...
            if (result !== null && result.corners !== null) {
                tlX = result.corners.topLeft.x;
                tlY = result.corners.topLeft.y;
                trX = result.corners.topRight.x;
                trY = result.corners.topRight.y;
                brX = result.corners.bottomRight.x;
                brY = result.corners.bottomRight.y;
                blX = result.corners.bottomLeft.x;
                blY = result.corners.bottomLeft.y;
                ...
                const { width: imgW, height: imgH } = await new Promise((resolve, reject) => Image.getSize(fileUri, (w, h) => resolve({ width: w, height: h }), reject));
            ...
            const rawUri = await NativeDocumentScanner.cropAndCorrectPerspective(photo.path, tlX, tlY, trX, trY, brX, brY, blX, blY);
            const normalizedUri = rawUri.startsWith('file://') ? rawUri : `file://${rawUri}`;
            const base64 = await NativeDocumentScanner.readFileAsBase64(normalizedUri.replace('file://', ''));
    ...
    const handleDetected = useCallback(() => {
        ...
        if (scanState === 'capturing' || scanState === 'processing' || scanState === 'preview')
            return;
        if (scanState === 'stabilizing')
            return;
    ...
    const handleNotDetected = useCallback(() => {
        ...
        if (Platform.OS === 'ios') {
            ...
            if (consecutiveMissesRef.current < 5)
                return;
        }
    ...
    return (<View style={styles.container}>
      ...
      <Camera key={cameraKey} ref={cameraRef} style={StyleSheet.absoluteFill} device={device} isActive={isActive} photo={true} photoQualityBalance="speed"/>
      ...
      {(scanState === 'searching' || scanState === 'detected' || scanState === 'stabilizing') && (<View style={styles.captureButtonContainer}>
          <TouchableOpacity style={styles.captureButton} onPress={handleManualCapture} activeOpacity={0.8}>
