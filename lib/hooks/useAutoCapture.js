...
export function useAutoCapture({ cameraRef, captureLockedRef, tierConfig, enabled, onDetected, onNotDetected, }) {
    ...
    const analyze = useCallback(async () => {
        ...
        if (captureLockedRef.current)
            return; // captura de calidad en progreso
        ...
        if (now - enabledAtRef.current < 1500)
            return;
        ...
        try {
            if (!enabledRef.current || captureLockedRef.current)
                return;
...
//# sourceMappingURL=useAutoCapture.js.map