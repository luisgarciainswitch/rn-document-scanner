 ...
 */
export async function scanDocumentFile(input, options = {}) {
    const { forceHorizontal = false, cameraViewLayout, guideFrameLayout, fallbackAspectRatio = 1.41, onNoDetection = 'fallback', } = options;
    ...
    if (detection !== null && detection.corners !== null) {
        ...
        if (cameraViewLayout && guideFrameLayout) {
            ...
            const intersectRight = Math.min(guideFrameLayout.left + guideFrameLayout.width, cameraViewLayout.left + cameraViewLayout.width);
            const intersectBottom = Math.min(guideFrameLayout.top + guideFrameLayout.height, cameraViewLayout.top + cameraViewLayout.height);
            ...
            if (onNoDetection === 'throw') {
                throw new Error('[scanDocumentFile] No se detectó ningún documento en la imagen.');
            }
    ...
    const rawUri = await NativeDocumentScanner.cropAndCorrectPerspective(filePath, corners.topLeft.x, corners.topLeft.y, corners.topRight.x, corners.topRight.y, corners.bottomRight.x, corners.bottomRight.y, corners.bottomLeft.x, corners.bottomLeft.y);
    const uri = rawUri.startsWith('file://') ? rawUri : `file://${rawUri}`;
    const base64 = await NativeDocumentScanner.readFileAsBase64(uri.replace('file://', ''));
    return {
        uri,
        base64,
